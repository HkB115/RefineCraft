...
public class RCprotectEntityListener implements Listener {
    private final InterfaceProtect prot;
    private final InterfaceRefineCraft ess;

    public RCprotectEntityListener(final InterfaceProtect prot) {
        this.prot = prot;
    ...
    public void onEntityDamage(final EntityDamageEvent event) {
        ...
        if (event instanceof EntityDamageByEntityEvent) {
            ...
            // PVP Settings
            if (target instanceof Player && eAttack instanceof Player && prot.getSettingBool(ProtectConfig.disable_pvp) && !user.getName().equalsIgnoreCase(attacker.getName()) && (!user.isAuthorized("refinecraft.protect.pvp") || !attacker.isAuthorized("refinecraft.protect.pvp"))) {
                event.setCancelled(true);
                return;
            }

            if (edEvent.getDamager() instanceof Projectile && target instanceof Player && ((prot.getSettingBool(ProtectConfig.disable_projectiles) && !shouldBeDamaged(user, "projectiles")) || (((Projectile) edEvent.getDamager()).getShooter() instanceof Player && prot.getSettingBool(ProtectConfig.disable_pvp) && (!user.isAuthorized("refinecraft.protect.pvp") || !ess.getUser((Player) ((Projectile) edEvent.getDamager()).getShooter()).isAuthorized("refinecraft.protect.pvp"))))) {
                event.setCancelled(true);
                return;
            }
        }
    ...
    private boolean shouldBeDamaged(final User user, final String type) {
        return (user.isAuthorized("refinecraft.protect.damage.".concat(type)) && !user.isAuthorized("refinecraft.protect.damage.disable"));
    }
    ...
    public void onEntityTarget(final EntityTargetEvent event) {
        ...
        if ((event.getReason() == TargetReason.CLOSEST_PLAYER || event.getReason() == TargetReason.TARGET_ATTACKED_ENTITY || event.getReason() == TargetReason.PIG_ZOMBIE_TARGET || event.getReason() == TargetReason.RANDOM_TARGET || event.getReason() == TargetReason.DEFEND_VILLAGE || event.getReason() == TargetReason.TARGET_ATTACKED_OWNER || event.getReason() == TargetReason.OWNER_ATTACKED_TARGET) && prot.getSettingBool(ProtectConfig.prevent_entitytarget) && !user.isAuthorized("refinecraft.protect.entitytarget.bypass")) {
            event.setCancelled(true);
        }
    }
