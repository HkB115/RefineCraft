...
public class Commandtime extends RefineCraftCommand {
    public Commandtime() {
        super("time");
    }
    ...
    public void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception {
        ...
        if (user != null && !user.isAuthorized("refinecraft.time.set")) {
    ...
    private boolean canUpdateAll(User user) {
        return !RC.getSettings().isWorldTimePermissions() // First check if per world permissions are enabled, if not, return true. 
            || user == null || user.isAuthorized("refinecraft.time.world.all");
    }

    private boolean canUpdateWorld(User user, World world) {
        return canUpdateAll(user) || user.isAuthorized("refinecraft.time.world." + normalizeWorldName(world));
    }
