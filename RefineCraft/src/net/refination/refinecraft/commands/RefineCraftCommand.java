...
public abstract class RefineCraftCommand implements InterfaceRefineCraftCommand {
    private final transient String name;
    protected transient InterfaceRefineCraft RC;
    protected transient InterfaceRefineCraftModule module;
    protected static final Logger logger = Logger.getLogger("RefineCraft");

    protected RefineCraftCommand(final String name) {
        this.name = name;
    }

    @Override
    public void setRefineCraft(final InterfaceRefineCraft RC) {
        this.RC = RC;
    }

    @Override
    public void setRefineCraftModule(final InterfaceRefineCraftModule module) {
        this.module = module;
    }
    ...
    // Get online players - only show ghosted if source has permission
    ...
    protected User getPlayer(final Server server, final CommandSource sender, final String searchTerm) throws PlayerNotFoundException, NotEnoughArgumentsException {
        if (sender.isPlayer()) {
            User user = RC.getUser(sender.getPlayer());
            return getPlayer(server, user, searchTerm, user.canInteractGhosted(), false);
        }
    ...
    protected User getPlayer(final Server server, final User user, final String[] args, final int pos) throws PlayerNotFoundException, NotEnoughArgumentsException {
        return getPlayer(server, user, args, pos, user.canInteractGhosted(), false);
    }
    ...
    private User getPlayer(final Server server, final User sourceUser, final String searchTerm, boolean getHidden, final boolean getOffline) throws PlayerNotFoundException {
        ...
        if (exPlayer != null) {
            user = RC.getUser(exPlayer);
        } else {
            user = RC.getUser(searchTerm);
        }
        ...
        if (matches.isEmpty()) {
            ...
            for (User userMatch : RC.getOnlineUsers()) {
    ...
    boolean canInteractWith(CommandSource interactor, User interactee) {
        ...
        if (interactor.isPlayer()) {
            return canInteractWith(RC.getUser(interactor.getPlayer()), interactee);
        }
