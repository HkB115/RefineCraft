...
public class Commandbalancetop extends RefineCraftCommand {
    ...
    protected void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception {
        ...
        if (args.length > 0) {
            try {
                ...
                if (args[0].equalsIgnoreCase("force") && (!sender.isPlayer() || RC.getUser(sender.getPlayer()).isAuthorized("refinecraft.balancetop.force"))) {
                    force = true;
                }
            }
        }
        ...
        if (!force && lock.readLock().tryLock()) {
            try {
                ...
                if (RC.getUserMap().getUniqueUsers() > MINUSERS) {
                    sender.sendMessage(tl("orderBalances", RC.getUserMap().getUniqueUsers()));
                }
            } finally {
                lock.readLock().unlock();
            }
            RC.runTaskAsynchronously(new Viewer(sender, commandLabel, page, force));
        } else {
            if (RC.getUserMap().getUniqueUsers() > MINUSERS) {
                sender.sendMessage(tl("orderBalances", RC.getUserMap().getUniqueUsers()));
            }
    ...
    private class Calculator implements Runnable {
        ...
        public void run() {
                ...
                if (force || cacheage <= System.currentTimeMillis() - CACHETIME) {
                    ...
                    if (RC.getSettings().isEcoDisabled()) {
                        if (RC.getSettings().isDebug()) {
                            RC.getLogger().info("Internal economy functions disabled, aborting baltop.");
                        }
                    } else {
                        for (UUID u : RC.getUserMap().getAllUniqueUsers()) {
                    ...
                    cache.getLines().add(tl("serverTotal", NumberUtil.displayCurrency(totalMoney, RC)));
                    int pos = 1;
                    for (Map.Entry<String, BigDecimal> entry : sortedEntries) {
                        cache.getLines().add(pos + ". " + entry.getKey() + ", " + NumberUtil.displayCurrency(entry.getValue(), RC));
                        pos++;
                    }
            ...
            RC.runTaskAsynchronously(viewer);
        }
    }
    ...
    private class Viewer implements Runnable {
        ...
        public void run() {
            ...
            RC.runTaskAsynchronously(new Calculator(new Viewer(sender, commandLabel, page, false), force));
        }
    }
}
