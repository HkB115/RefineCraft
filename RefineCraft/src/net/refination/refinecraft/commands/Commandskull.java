...
public class Commandskull extends RefineCraftCommand {
    public Commandskull() {
        super("skull");
    }
    ...
    protected void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception {
        ...
        if (args.length > 0 && user.isAuthorized("refinecraft.skull.others")) {
        ...
        } else if (user.isAuthorized("refinecraft.skull.spawn")) {
        ...
        if (metaSkull.hasOwner() && !user.isAuthorized("refinecraft.skull.modify")) {
