...
public class Commandpowertool extends RefineCraftCommand {
    ...
    protected void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception {
        ...
        final ItemStack itemStack = RC.getItemDb().get(args[1]);
        ...
        if (command != null && !command.isEmpty()) {
            ...
            if (command.startsWith("r:")) {
                ...
                if (command.startsWith("a:")) {
                    if (sender.isPlayer() && !RC.getUser(sender.getPlayer()).isAuthorized("refinecraft.powertool.append")) {
                        throw new Exception(tl("noPerm", "refinecraft.powertool.append"));
                    }
                    command = command.substring(2);
                    if (powertools.contains(command)) {
