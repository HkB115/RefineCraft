...
public class Commandworth extends RefineCraftCommand {
    public Commandworth() {
        super("worth");
    }
    ...
    public void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception {
        ...
        if (count > 1) {
            if (args.length > 0 && args[0].equalsIgnoreCase("blocks")) {
                user.sendMessage(tl("totalSellableBlocks", type, NumberUtil.displayCurrency(totalWorth, RC)));
            } else {
                user.sendMessage(tl("totalSellableAll", type, NumberUtil.displayCurrency(totalWorth, RC)));
            }
        }
    }
    ...
    private BigDecimal itemWorth(CommandSource sender, User user, ItemStack is, String[] args) throws Exception {
        ...
        if (user == null) {
            ...
            amount = RC.getWorth().getAmount(RC, user, is, args, true);
        }
        ...
        sender.sendMessage(is.getDurability() != 0 ? tl("worthMeta", is.getType().toString().toLowerCase(Locale.ENGLISH).replace("_", ""), is.getDurability(), NumberUtil.displayCurrency(result, RC), amount, NumberUtil.displayCurrency(worth, RC)) : tl("worth", is.getType().toString().toLowerCase(Locale.ENGLISH).replace("_", ""), NumberUtil.displayCurrency(result, RC), amount, NumberUtil.displayCurrency(worth, RC)));
