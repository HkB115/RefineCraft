...
public class Commandptime extends RefineCraftCommand {
    ...
    public void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception {
        ...
        if (sender.isPlayer()) {
            ...
            if (user != null && (!users.contains(user) || users.size() > 1) && !user.isAuthorized("refinecraft.ptime.others")) {
    ...
    private Set<User> getUsers(final Server server, final CommandSource sender, final String selector) throws Exception {
        ...
        if (!matchedPlayers.isEmpty()) {
            user = RC.getUser(matchedPlayers.get(0));
        }
