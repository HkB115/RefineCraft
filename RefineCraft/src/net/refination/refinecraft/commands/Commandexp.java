...
public class Commandexp extends RefineCraftCommand {
    public Commandexp() {
        super("exp");
    }
    ...
    public void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception {
        if (args.length == 0) {
        ...
        } else if (args.length > 1 && args[0].equalsIgnoreCase("set") && user.isAuthorized("refinecraft.exp.set")) {
            if (args.length == 3 && user.isAuthorized("refinecraft.exp.set.others")) {
                expMatch(server, user.getSource(), args[1], args[2], false);
            } else {
                setExp(user.getSource(), user, args[1], false);
            }
        } else if (args.length > 1 && args[0].equalsIgnoreCase("give") && user.isAuthorized("refinecraft.exp.give")) {
            if (args.length == 3 && user.isAuthorized("refinecraft.exp.give.others")) {
                expMatch(server, user.getSource(), args[1], args[2], true);
            } else {
                setExp(user.getSource(), user, args[1], true);
            }
        } else if (args[0].equalsIgnoreCase("show")) {
            if (args.length >= 2 && user.isAuthorized("refinecraft.exp.others")) {
            ...
            if (args.length >= 1 && NumberUtil.isInt(args[0].toLowerCase(Locale.ENGLISH).replace("l", "")) && user.isAuthorized("refinecraft.exp.give")) {
                if (args.length >= 2 && user.isAuthorized("refinecraft.exp.give.others")) {
                    expMatch(server, user.getSource(), args[1], args[0], true);
                } else {
                    setExp(user.getSource(), user, args[0], true);
                }
            } else if (args.length >= 1 && user.isAuthorized("refinecraft.exp.others")) {
