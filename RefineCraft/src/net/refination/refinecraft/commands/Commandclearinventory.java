...
public class Commandclearinventory extends RefineCraftCommand {
    ...
    public void run(Server server, User user, String commandLabel, String[] args) throws Exception {
        parseCommand(server, user.getSource(), args, user.isAuthorized("refinecraft.clearinventory.others"), user.isAuthorized("refinecraft.clearinventory.all") || user.isAuthorized("refinecraft.clearinventory.multiple"));
    }
    ...
    protected void clearHandler(CommandSource sender, Player player, String[] args, int offset, boolean showExtended) throws Exception {
        ...
        if (args.length > offset) {
            if (args[offset].equalsIgnoreCase("**")) {
                ...
                final ItemStack item = RC.getItemDb().get(split[0]);
                type = item.getTypeId();
