...
import net.refination.refinecraft.textreader.BookInput;
import net.refination.refinecraft.textreader.BookPager;
...
public class MetaItemStack {
    ...
    public boolean canSpawn(final InterfaceRefineCraft RC) {
        try {
            RC.getServer().getUnsafe().modifyItemStack(stack.clone(), "{}");
            return true;
        } catch (NullPointerException npe) {
            if (RC.getSettings().isDebug()) {
                RC.getLogger().log(Level.INFO, "Itemstack is invalid", npe);
            }
            return false;
            ...
            if (RC.getSettings().isDebug()) {
                RC.getLogger().log(Level.INFO, "Itemstack is invalid", throwable);
            }
            return false;
        }
    }

    public void parseStringMeta(final CommandSource sender, final boolean allowUnsafe, String[] string, int fromArg, final InterfaceRefineCraft RC) throws Exception {
        if (string[fromArg].startsWith("{") && hasMetaPermission(sender, "vanilla", false, true, RC)) {
            try {
                stack = RC.getServer().getUnsafe().modifyItemStack(stack, Joiner.on(' ').join(Arrays.asList(string).subList(fromArg, string.length)));
            } catch (NullPointerException npe) {
                if (RC.getSettings().isDebug()) {
                    RC.getLogger().log(Level.INFO, "Itemstack is invalid", npe);
                }
            ...
            for (int i = fromArg; i < string.length; i++) {
                addStringMeta(sender, allowUnsafe, string[i], RC);
            }
            if (validFirework) {
                if (!hasMetaPermission(sender, "firework", true, true, RC)) {
                    throw new Exception(tl("noMetaFirework"));
                }
                FireworkEffect effect = builder.build();
                FireworkMeta fmeta = (FireworkMeta) stack.getItemMeta();
                fmeta.addEffect(effect);
                if (fmeta.getEffects().size() > 1 && !hasMetaPermission(sender, "firework-multiple", true, true, RC)) {
    ...
    public void addStringMeta(final CommandSource sender, final boolean allowUnsafe, final String string, final InterfaceRefineCraft RC) throws Exception {
        ...
        if (split.length > 1 && split[0].equalsIgnoreCase("name") && hasMetaPermission(sender, "name", false, true, RC)) {
            final String displayName = FormatUtil.replaceFormat(split[1].replace('_', ' '));
            final ItemMeta meta = stack.getItemMeta();
            meta.setDisplayName(displayName);
            stack.setItemMeta(meta);
        } else if (split.length > 1 && (split[0].equalsIgnoreCase("lore") || split[0].equalsIgnoreCase("desc")) && hasMetaPermission(sender, "lore", false, true, RC)) {
            ...
            meta.setLore(lore);
            stack.setItemMeta(meta);
        } else if (split.length > 1 && (split[0].equalsIgnoreCase("unbreakable") && hasMetaPermission(sender, "unbreakable", false, true, RC))) {
            setUnbreakable(stack, Boolean.valueOf(split[1]));
        } else if (split.length > 1 && (split[0].equalsIgnoreCase("player") || split[0].equalsIgnoreCase("owner")) && stack.getType() == Material.SKULL_ITEM && hasMetaPermission(sender, "head", false, true, RC)) {
        ...
        } else if (split.length > 1 && split[0].equalsIgnoreCase("book") && stack.getType() == Material.WRITTEN_BOOK && (hasMetaPermission(sender, "book", true, true, RC) || hasMetaPermission(sender, "chapter-" + split[1].toLowerCase(Locale.ENGLISH), true, true, RC))) {
            final BookMeta meta = (BookMeta) stack.getItemMeta();
            final InterfaceText input = new BookInput("book", true, RC);
            final BookPager pager = new BookPager(input);

            List<String> pages = pager.getPages(split[1]);
            meta.setPages(pages);
            stack.setItemMeta(meta);
        } else if (split.length > 1 && split[0].equalsIgnoreCase("author") && stack.getType() == Material.WRITTEN_BOOK && hasMetaPermission(sender, "author", false, true, RC)) {
            final String author = FormatUtil.replaceFormat(split[1]);
            final BookMeta meta = (BookMeta) stack.getItemMeta();
            meta.setAuthor(author);
            stack.setItemMeta(meta);
        } else if (split.length > 1 && split[0].equalsIgnoreCase("title") && stack.getType() == Material.WRITTEN_BOOK && hasMetaPermission(sender, "title", false, true, RC)) {
            final String title = FormatUtil.replaceFormat(split[1].replace('_', ' '));
            final BookMeta meta = (BookMeta) stack.getItemMeta();
            meta.setTitle(title);
            stack.setItemMeta(meta);
        } else if (split.length > 1 && split[0].equalsIgnoreCase("power") && stack.getType() == Material.FIREWORK && hasMetaPermission(sender, "firework-power", false, true, RC)) {
            ...
            parseEnchantmentStrings(sender, allowUnsafe, split, RC);
        }
    }

    public void addFireworkMeta(final CommandSource sender, final boolean allowShortName, final String string, final InterfaceRefineCraft RC) throws Exception {
    ...
    public void addPotionMeta(final CommandSource sender, final boolean allowShortName, final String string, final InterfaceRefineCraft RC) throws Exception {
        if (isPotion(stack.getType())) {
            ...
            if (split[0].equalsIgnoreCase("effect") || (allowShortName && split[0].equalsIgnoreCase("e"))) {
                ...
                if (pEffectType != null && pEffectType.getName() != null) {
                    if (hasMetaPermission(sender, "potions." + pEffectType.getName().toLowerCase(Locale.ENGLISH), true, false, RC)) {
            ...
            if (isValidPotion()) {
                ...
                if (pmeta.getCustomEffects().size() > 1 && !hasMetaPermission(sender, "potions.multiple", true, false, RC)) {
    ...
    private void parseEnchantmentStrings(final CommandSource sender, final boolean allowUnsafe, final String[] split, final InterfaceRefineCraft RC) throws Exception {
        final Enchantment enchantment = Enchantments.getByName(split[0]);
        if (enchantment == null || !hasMetaPermission(sender, "enchantments." + enchantment.getName().toLowerCase(Locale.ENGLISH), false, false, RC)) {
            return;
        }
    ...
    public void addBannerMeta(final CommandSource sender, final boolean allowShortName, final String string, final InterfaceRefineCraft RC) throws Exception {
    ...
    private boolean hasMetaPermission(final CommandSource sender, final String metaPerm, final boolean graceful, final boolean includeBase, final InterfaceRefineCraft RC) throws Exception {
        final User user = sender != null && sender.isPlayer() ? RC.getUser(sender.getPlayer()) : null;
    ...
    private boolean hasMetaPermission(final User user, final String metaPerm, final boolean graceful, final boolean includeBase) throws Exception {
        final String permBase = includeBase ? "refinecraft.itemspawn.meta-" : "refinecraft.";
