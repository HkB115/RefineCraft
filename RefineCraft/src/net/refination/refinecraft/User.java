...
import net.refination.refinecraft.register.payment.Method;
...
import net.refination.api.events.AfkStatusChangeEvent;
import net.refination.api.events.JailStatusChangeEvent;
import net.refination.api.events.UserBalanceUpdateEvent;
...
public class User extends UserData implements Comparable<User>, InterfaceMessageRecipient, net.refination.api.InterfaceUser {
    private static final Logger logger = Logger.getLogger("RefineCraft");
    private InterfaceMessageRecipient messageRecipient;
    private transient UUID teleportRequester;
    private transient boolean teleportRequestHere;
    private transient Location teleportLocation;
    private transient boolean ghosted;
    ...
    public User(final Player base, final InterfaceRefineCraft RC) {
        super(base, RC);
        teleport = new Teleport(this, RC);
        if (isAfk()) {
    ...
    @Override
    public boolean isAuthorized(final InterfaceRefineCraftCommand cmd) {
        return isAuthorized(cmd, "refinecraft.");
    }

    @Override
    public boolean isAuthorized(final InterfaceRefineCraftCommand cmd, final String permissionPrefix) {
    ...
    public boolean isAuthorized(final String node) {
        ...
        if (RC.getSettings().isDebug()) {
            RC.getLogger().log(Level.INFO, "checking if " + base.getName() + " has " + node + " - " + result);
        }
        return result;
    }
    ...
    private boolean isAuthorizedCheck(final String node) {
        ...
        try {
            return RC.getPermissionsHandler().hasPermission(base, node);
        } catch (Exception ex) {
            if (RC.getSettings().isDebug()) {
                RC.getLogger().log(Level.SEVERE, "Permission System Error: " + RC.getPermissionsHandler().getName() + " returned: " + ex.getMessage(), ex);
            } else {
                RC.getLogger().log(Level.SEVERE, "Permission System Error: " + RC.getPermissionsHandler().getName() + " returned: " + ex.getMessage());
            }
    ...
    public void healCooldown() throws Exception {
        ...
        if (getLastHealTimestamp() > 0) {
            final double cooldown = RC.getSettings().getHealCooldown();
            final Calendar cooldownTime = new GregorianCalendar();
            cooldownTime.setTimeInMillis(getLastHealTimestamp());
            cooldownTime.add(Calendar.SECOND, (int) cooldown);
            cooldownTime.add(Calendar.MILLISECOND, (int) ((cooldown * 1000.0) % 1000.0));
            if (cooldownTime.after(now) && !isAuthorized("refinecraft.heal.cooldown.bypass")) {
    ...
    public void giveMoney(final BigDecimal value, final CommandSource initiator) throws MaxMoneyException {
        ...
        sendMessage(tl("addedToAccount", NumberUtil.displayCurrency(value, RC)));
        if (initiator != null) {
            initiator.sendMessage(tl("addedToOthersAccount", NumberUtil.displayCurrency(value, RC), this.getDisplayName(), NumberUtil.displayCurrency(getMoney(), RC)));
        }
    }
    ...
    public void payUser(final User reciever, final BigDecimal value) throws ChargeException, MaxMoneyException {
        ...
        if (canAfford(value)) {
            ...
            sendMessage(tl("moneySentTo", NumberUtil.displayCurrency(value, RC), reciever.getDisplayName()));
            reciever.sendMessage(tl("moneyRecievedFrom", NumberUtil.displayCurrency(value, RC), getDisplayName()));
        } else {
            throw new ChargeException(tl("notEnoughMoney", NumberUtil.displayCurrency(value, RC)));
        }
    }
    ...
    public void takeMoney(final BigDecimal value, final CommandSource initiator) {
        ...
        try {
            ...
            RC.getLogger().log(Level.WARNING, "Invalid call to takeMoney, total balance can't be more than the max-money limit.", ex);
        }
        sendMessage(tl("takenFromAccount", NumberUtil.displayCurrency(value, RC)));
        if (initiator != null) {
            initiator.sendMessage(tl("takenFromOthersAccount", NumberUtil.displayCurrency(value, RC), this.getDisplayName(), NumberUtil.displayCurrency(getMoney(), RC)));
        }
    }
    ...
    public boolean canAfford(final BigDecimal cost, final boolean permcheck) {
        ...
        if (!permcheck || isAuthorized("refinecraft.eco.loan")) {
            return (remainingBalance.compareTo(RC.getSettings().getMinMoney()) >= 0);
        }
    ...
    private void _dispose() {
        if (!base.isOnline()) {
            this.base = new OfflinePlayer(getConfigUUID(), RC.getServer());
        }
        cleanup();
    }
    ...
    public Boolean canSpawnItem(final int itemId) {
        return !RC.getSettings().itemSpawnBlacklist().contains(itemId);
    }
    ...
    public String getNick(final boolean longnick) {
        ...
        if (RC.getSettings().isCommandDisabled("nick") || nick == null || nick.isEmpty() || nick.equals(getName())) {
            nickname = getName();
        } else if (nick.equalsIgnoreCase(getName())) {
            nickname = nick;
        } else {
            nickname = RC.getSettings().getNicknamePrefix() + nick;
            suffix = "§r";
        }

        if (this.getBase().isOp()) {
            try {
                final ChatColor opPrefix = RC.getSettings().getOperatorColor();
        ...
        if (RC.getSettings().addPrefixSuffix()) {
            //These two extra toggles are not documented, because they are mostly redundant #EasterEgg
            if (!RC.getSettings().disablePrefix()) {
                final String ptext = RC.getPermissionsHandler().getPrefix(base).replace('&', '§');
                prefix.insert(0, ptext);
                suffix = "§r";
            }
            if (!RC.getSettings().disableSuffix()) {
                final String stext = RC.getPermissionsHandler().getSuffix(base).replace('&', '§');
                suffix = stext + "§r";
    ...
    public void setDisplayNick() {
        if (base.isOnline() && RC.getSettings().changeDisplayName()) {
            this.getBase().setDisplayName(getNick(true));
            if (RC.getSettings().changePlayerListName()) {
    ...
    public BigDecimal getMoney() {
        ...
        if (elapsed > RC.getSettings().getEconomyLagWarning()) {
            RC.getLogger().log(Level.INFO, "Lag Notice - Slow Economy Response - Request took over {0}ms!", elapsed / 1000000.0);
        }
        return value;
    }
    ...
    private BigDecimal _getMoney() {
        if (RC.getSettings().isEcoDisabled()) {
            if (RC.getSettings().isDebug()) {
                RC.getLogger().info("Internal economy functions disabled, aborting balance check.");
            }
    ...
    public void setMoney(final BigDecimal value) throws MaxMoneyException {
        if (RC.getSettings().isEcoDisabled()) {
            if (RC.getSettings().isDebug()) {
                RC.getLogger().info("Internal economy functions disabled, aborting balance change.");
            }
            return;
        }
        ...
        RC.getServer().getPluginManager().callEvent(new UserBalanceUpdateEvent(this.getBase(), oldBalance, value));
        Trade.log("Update", "Set", "API", getName(), new Trade(value, RC), null, null, null, RC);
    }
    ...
    public void setAfk(final boolean set) {
        ...
        RC.getServer().getPluginManager().callEvent(afkEvent);
        if (afkEvent.isCancelled()) {
            return;
        }

        this.getBase().setSleepingIgnored(this.isAuthorized("refinecraft.sleepingignored") ? true : set);
        if (set && !isAfk()) {
            afkPosition = this.getLocation();
        } else if (!set && isAfk()) {
            afkPosition = null;
        }
        if (RC.getSettings().isAfkListName()) {
            if(set) {
                String afkName = RC.getSettings().getAfkListName().replace("{PLAYER}", getDisplayName()).replace("{USERNAME}", getName());
    ...
    public void updateActivity(final boolean broadcast) {
        if (isAfk() && RC.getSettings().cancelAfkOnInteract()) {
            setAfk(false);
            if (broadcast && !isHidden()) {
                setDisplayNick();
                final String msg = tl("userIsNotAway", getDisplayName());
                if (!msg.isEmpty()) {
                    RC.broadcastMessage(this, msg);
                }
            }
        }
    ...
    public void checkActivity() {
        ...
        final long autoafkkick = RC.getSettings().getAutoAfkKick();
        if (autoafkkick > 0 && lastActivity > 0 && (lastActivity + (autoafkkick * 1000)) < System.currentTimeMillis() && !isHidden() && !isAuthorized("refinecraft.kick.exempt") && !isAuthorized("refinecraft.afk.kickexempt")) {
            final String kickReason = tl("autoAfkKickReason", autoafkkick / 60.0);
            lastActivity = 0;
            this.getBase().kickPlayer(kickReason);


            for (User user : RC.getOnlineUsers()) {
                if (user.isAuthorized("refinecraft.kick.notify")) {
                    user.sendMessage(tl("playerKicked", Console.NAME, getName(), kickReason));
                }
            }
        }
        final long autoafk = RC.getSettings().getAutoAfk();
        if (!isAfk() && autoafk > 0 && lastActivity + autoafk * 1000 < System.currentTimeMillis() && isAuthorized("refinecraft.afk.auto")) {
            setAfk(true);
            if (!isHidden()) {
                setDisplayNick();
    ...
    public boolean isGodModeEnabled() {
        return (super.isGodModeEnabled() && !RC.getSettings().getNoGodWorlds().contains(this.getLocation().getWorld().getName())) || (isAfk() && RC.getSettings().getFreezeAfkPlayers());
    }
    ...
    public String getGroup() {
        final String result = RC.getPermissionsHandler().getGroup(base);
        if (RC.getSettings().isDebug()) {
            RC.getLogger().log(Level.INFO, "looking up groupname of " + base.getName() + " - " + result);
        }
        return result;
    }
    ...
    public boolean inGroup(final String group) {
        final boolean result = RC.getPermissionsHandler().inGroup(base, group);
        if (RC.getSettings().isDebug()) {
            RC.getLogger().log(Level.INFO, "checking if " + base.getName() + " is in group " + group + " - " + result);
        }
        return result;
    }
    ...
    public boolean canBuild() {
        ...
        return RC.getPermissionsHandler().canBuild(base, getGroup());
    }
    ...
    public void enableInvulnerabilityAfterTeleport() {
        final long time = RC.getSettings().getTeleportInvulnerability();
        if (time > 0) {
    ...
    public boolean canInteractGhosted() {
        return isAuthorized("refinecraft.ghost.interact");
    }
    ...
    @Override
    public void setGhosted(final boolean set) {
        ghosted = set;
        if (set) {
            for (User user : RC.getOnlineUsers()) {
                if (!user.isAuthorized("refinecraft.ghost.see")) {
                    user.getBase().hidePlayer(getBase());
                }
            }
            setHidden(true);
            RC.getGhostedPlayers().add(getName());
            if (isAuthorized("refinecraft.ghost.effect")) {
                this.getBase().addPotionEffect(new PotionEffect(PotionEffectType.INVISIBILITY, Integer.MAX_VALUE, 1, false));
            }
        } else {
            for (Player p : RC.getOnlinePlayers()) {
                p.showPlayer(getBase());
            }
            setHidden(false);
            RC.getGhostedPlayers().remove(getName());
            if (isAuthorized("refinecraft.ghost.effect")) {
    ...
    public boolean isSignThrottled() {
        final long minTime = lastThrottledAction + (1000 / RC.getSettings().getSignUsePerSecond());
    ...
    public boolean isIgnoreExempt() {
        return this.isAuthorized("refinecraft.chat.ignoreexempt");
    }
