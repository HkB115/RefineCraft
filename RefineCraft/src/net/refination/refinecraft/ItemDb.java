...
public class ItemDb implements InterfaceConf, net.refination.api.InterfaceItemDb {
    ...
    public ItemDb(final InterfaceRefineCraft RC) {
        this.RC = RC;
        file = new ManagedFile("items.csv", RC);
    }
    ...
    public ItemStack get(final String id) throws Exception {
        ...
        if (mat == Material.MOB_SPAWNER) {
            ...
            try {
                retval = RC.getSpawnerProvider().setEntityType(retval, EntityType.fromId(metaData));
            ...
            retval = RC.getSpawnEggProvider().createEggItem(type);
        } else if (mat.name().endsWith("POTION")) {
            retval = RC.getPotionMetaProvider().createPotionItem(mat, metaData);
        } else {
    ...
    public String serialize(ItemStack is) {
        ...
        switch (is.getType()) {
            case WRITTEN_BOOK:
                // Everything from http://wiki.refination.net/wiki/Item_Meta#Books in that order.
            ...
            case ENCHANTED_BOOK:
                ...
                break;
            case FIREWORK:
                // Everything from http://wiki.refination.net/wiki/Item_Meta#Fireworks in that order.
