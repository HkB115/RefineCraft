...
public class Trade {
    ...
    public Trade(final String command, final InterfaceRefineCraft RC) {
        this(command, null, null, null, null, RC);
    }

    public Trade(final String command, final Trade fallback, final InterfaceRefineCraft RC) {
        this(command, fallback, null, null, null, RC);
    }

    @Deprecated
    public Trade(final double money, final net.refination.refinecraft.InterfaceRefineCraft RC) {
        this(null, null, BigDecimal.valueOf(money), null, null, (InterfaceRefineCraft) RC);
    }

    public Trade(final BigDecimal money, final InterfaceRefineCraft RC) {
        this(null, null, money, null, null, RC);
    }

    public Trade(final ItemStack items, final InterfaceRefineCraft RC) {
        this(null, null, null, items, null, RC);
    }

    public Trade(final int exp, final InterfaceRefineCraft RC) {
        this(null, null, null, null, exp, RC);
    }

    private Trade(final String command, final Trade fallback, final BigDecimal money, final ItemStack item, final Integer exp, final InterfaceRefineCraft RC) {
    ...
    public void isAffordableFor(final InterfaceUser user) throws ChargeException {

        if (RC.getSettings().isDebug()) {
            RC.getLogger().log(Level.INFO, "checking if " + user.getName() + " can afford charge.");
        }

        if (getMoney() != null && getMoney().signum() > 0 && !user.canAfford(getMoney())) {
            throw new ChargeException(tl("notEnoughMoney", NumberUtil.displayCurrency(getMoney(), RC)));
        }

        if (getItemStack() != null && !user.getBase().getInventory().containsAtLeast(itemStack, itemStack.getAmount())) {
            throw new ChargeException(tl("missingItems", getItemStack().getAmount(), RC.getItemDb().name(getItemStack())));
        }

        BigDecimal money;
        if (command != null && !command.isEmpty() && (money = getCommandCost(user)).signum() > 0 && !user.canAfford(money)) {
            throw new ChargeException(tl("notEnoughMoney", NumberUtil.displayCurrency(money, RC)));
        }
    ...
    public boolean pay(final InterfaceUser user) throws MaxMoneyException {
        return pay(user, OverflowType.ABORT) == null;
    }

    public Map<Integer, ItemStack> pay(final InterfaceUser user, final OverflowType type) throws MaxMoneyException {
        if (getMoney() != null && getMoney().signum() > 0) {
            if (RC.getSettings().isDebug()) {
                RC.getLogger().log(Level.INFO, "paying user " + user.getName() + " via trade " + getMoney().toPlainString());
            }
        ...
        if (getItemStack() != null) {
            ...
            if (overFlow != null) {
                switch (type) {
                    case ABORT:
                        if (RC.getSettings().isDebug()) {
                            RC.getLogger().log(Level.INFO, "abort paying " + user.getName() + " itemstack " + getItemStack().toString() + " due to lack of inventory space ");
                        }
                    ...
                    case RETURN:
                        ...
                        if (RC.getSettings().isDebug()) {
                            RC.getLogger().log(Level.INFO, "paying " + user.getName() + " partial itemstack " + getItemStack().toString() + " with overflow " + returnStack.get(0).toString());
                        }
                    ...
                    case DROP:
                        ...
                        if (RC.getSettings().isDebug()) {
                            RC.getLogger().log(Level.INFO, "paying " + user.getName() + " partial itemstack " + getItemStack().toString() + " and dropping overflow " + leftOver.get(0).toString());
                        }
                }
            } else if (RC.getSettings().isDebug()) {
                RC.getLogger().log(Level.INFO, "paying " + user.getName() + " itemstack " + getItemStack().toString());
            }
            user.getBase().updateInventory();
        }
    ...
    public void charge(final InterfaceUser user) throws ChargeException {
        if (RC.getSettings().isDebug()) {
            RC.getLogger().log(Level.INFO, "attempting to charge user " + user.getName());
        }
        if (getMoney() != null) {
            if (RC.getSettings().isDebug()) {
                RC.getLogger().log(Level.INFO, "charging user " + user.getName() + " money " + getMoney().toPlainString());
            }
        ...
        if (getItemStack() != null) {
            if (RC.getSettings().isDebug()) {
                RC.getLogger().log(Level.INFO, "charging user " + user.getName() + " itemstack " + getItemStack().toString());
            }
        ...
        if (command != null) {
            ...
            if (!user.canAfford(cost) && cost.signum() > 0) {
                throw new ChargeException(tl("notEnoughMoney", NumberUtil.displayCurrency(cost, RC)));
            }
            user.takeMoney(cost);
        }
        if (getExperience() != null) {
            if (RC.getSettings().isDebug()) {
                RC.getLogger().log(Level.INFO, "charging user " + user.getName() + " exp " + getExperience());
            }
        ...
        if (RC.getSettings().isDebug()) {
            RC.getLogger().log(Level.INFO, "charge user " + user.getName() + " completed");
        }
    }
    ...
    public BigDecimal getCommandCost(final InterfaceUser user) {
        BigDecimal cost = BigDecimal.ZERO;
        if (command != null && !command.isEmpty()) {
            cost = RC.getSettings().getCommandCost(command.charAt(0) == '/' ? command.substring(1) : command);
            if (cost.signum() == 0 && fallbackTrade != null) {
                cost = fallbackTrade.getCommandCost(user);
            }

            if (RC.getSettings().isDebug()) {
                RC.getLogger().log(Level.INFO, "calculated command (" + command + ") cost for " + user.getName() + " as " + cost);
            }
        }
        if (cost.signum() != 0 && (user.isAuthorized("refinecraft.nocommandcost.all") || user.isAuthorized("refinecraft.nocommandcost." + command))) {
    ...
    public static void log(String type, String subtype, String event, String sender, Trade charge, String receiver, Trade pay, Location loc, InterfaceRefineCraft RC) {
        ...
        if ((loc == null && !RC.getSettings().isEcoLogUpdateEnabled()) || (loc != null && !RC.getSettings().isEcoLogEnabled())) {
            return;
        }
        if (fw == null) {
            try {
                fw = new FileWriter(new File(RC.getDataFolder(), "trade.log"), true);
            } catch (IOException ex) {
                Logger.getLogger("RefineCraft").log(Level.SEVERE, null, ex);
            }
        }
        ...
        if (charge == null) {
            ...
            if (charge.getMoney() != null) {
                ...
                sb.append(RC.getSettings().getCurrencySymbol());
            }
