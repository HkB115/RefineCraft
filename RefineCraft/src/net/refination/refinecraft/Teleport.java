...
public class Teleport implements net.refination.api.InterfaceTeleport {
    ...
    public Teleport(InterfaceUser user, InterfaceRefineCraft RC) {
    ...
    public void cooldown(boolean check) throws Exception {
        ...
        if (teleportOwner.getLastTeleportTimestamp() > 0) {
            ...
            final double cooldown = RC.getSettings().getTeleportCooldown();
            ...
            if (lastTime > time.getTimeInMillis()) {
                // This is to make sure time didn't get mRCed up on last teleportPlayer use.
    ...
    private boolean cooldownApplies() {
        ...
        String globalBypassPerm = "refinecraft.teleport.cooldown.bypass";
        switch (tpType) {
            case NORMAL:
            ...
            case BACK:
                applies = !(teleportOwner.isAuthorized(globalBypassPerm) &&
                        teleportOwner.isAuthorized("refinecraft.teleport.cooldown.bypass.back"));
                break;
            case TPA:
                applies = !(teleportOwner.isAuthorized(globalBypassPerm) &&
                        teleportOwner.isAuthorized("refinecraft.teleport.cooldown.bypass.tpa"));
                break;
        }
        return applies;
    }

    private void warnUser(final InterfaceUser user, final double delay) {
    ...
    public void now(Location loc, boolean cooldown, TeleportCause cause) throws Exception {
        ...
        final InterfaceTarget target = new LocationTarget(loc);
    ...
    public void now(Player entity, boolean cooldown, TeleportCause cause) throws Exception {
        ...
        final InterfaceTarget target = new PlayerTarget(entity);
        now(teleportOwner, target, cause);
        teleportOwner.sendMessage(tl("teleporting", target.getLocation().getWorld().getName(), target.getLocation().getBlockX(), target.getLocation().getBlockY(), target.getLocation().getBlockZ()));
    }

    protected void now(InterfaceUser teleportee, InterfaceTarget target, TeleportCause cause) throws Exception {
        cancel(false);
        teleportee.setLastLocation();
        final Location loc = target.getLocation();

        if (LocationUtil.isBlockUnsafeForUser(teleportee, loc.getWorld(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ())) {
            if (RC.getSettings().isTeleportSafetyEnabled()) {
                if (teleportee.getBase().isInsideVehicle()) {
                    teleportee.getBase().leaveVehicle();
                }
                if (RC.getSettings().isForceDisableTeleportSafety()) {
    ...
    public void teleport(Player entity, Trade chargeFor, TeleportCause cause) throws Exception {
        InterfaceTarget target = new PlayerTarget(entity);
    ...
    @Override
    public void teleportPlayer(InterfaceUser teleportee, Location loc, Trade chargeFor, TeleportCause cause) throws Exception {
        teleport(teleportee, new LocationTarget(loc), chargeFor, cause);
    }

    //This is used on /tphere
    @Override
    public void teleportPlayer(InterfaceUser teleportee, Player entity, Trade chargeFor, TeleportCause cause) throws Exception {
        InterfaceTarget target = new PlayerTarget(entity);
    ...
    private void teleport(InterfaceUser teleportee, InterfaceTarget target, Trade chargeFor, TeleportCause cause) throws Exception {
        double delay = RC.getSettings().getTeleportDelay();
        ...
        if (chargeFor != null) {
            ...
            if (!chargeFor.getCommandCost(teleportOwner).equals(BigDecimal.ZERO)) {
                ...
                cashCharge = new Trade(chargeFor.getCommandCost(teleportOwner), RC);
            }
        }

        cooldown(true);
        if (delay <= 0 || teleportOwner.isAuthorized("refinecraft.teleport.timer.bypass") || teleportee.isAuthorized("refinecraft.teleport.timer.bypass")) {
            cooldown(false);
    ...
    public void respawn(final Trade chargeFor, TeleportCause cause) throws Exception {
        ...
        cooldown(true);
        if (delay <= 0 || teleportOwner.isAuthorized("refinecraft.teleport.timer.bypass")) {
            cooldown(false);
    ...
    protected void respawnNow(InterfaceUser teleportee, TeleportCause cause) throws Exception {
        ...
        if (bed != null) {
            ...
            if (RC.getSettings().isDebug()) {
                RC.getLogger().info("Could not find bed spawn, forcing respawn event.");
            }
            final PlayerRespawnEvent pre = new PlayerRespawnEvent(player, player.getWorld().getSpawnLocation(), false);
            RC.getServer().getPluginManager().callEvent(pre);
    ...
    @Override
    public void warp(InterfaceUser teleportee, String warp, Trade chargeFor, TeleportCause cause) throws Exception {
        Location loc = RC.getWarps().getWarp(warp);
    ...
    private void initTimer(long delay, InterfaceUser teleportUser, InterfaceTarget target, Trade chargeFor, TeleportCause cause, boolean respawn) {
        timedTeleport = new TimedTeleport(teleportOwner, RC, this, delay, teleportUser, target, chargeFor, cause, respawn);
    }
}
