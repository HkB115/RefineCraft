...
import net.refination.refinecraft.RefineCraftUserConf;
...
public class Economy {
    ...
    private static InterfaceRefineCraft RC;
    private static final String noCallBeforeLoad = "RefineCraft API is called before RefineCraft is loaded.";
    public static final MathContext MATH_CONTEXT = MathContext.DECIMAL128;

    /**
     * @param aRC the RC to set
     */
    public static void setRC(InterfaceRefineCraft aRC) {
        RC = aRC;
    }

    private static void createNPCFile(String name) {
        File folder = new File(RC.getDataFolder(), "userdata");
        name = StringUtil.safeString(name);
        if (!folder.exists()) {
            folder.mkdirs();
        }
        UUID npcUUID = UUID.nameUUIDFromBytes(("NPC:" + name).getBytes(Charsets.UTF_8));
        RefineCraftUserConf npcConfig = new RefineCraftUserConf(name, npcUUID, new File(folder, npcUUID.toString() + ".yml"));
        npcConfig.load();
        npcConfig.setProperty("npc", true);
        npcConfig.setProperty("lastAccountName", name);
        npcConfig.setProperty("money", RC.getSettings().getStartingBalance());
        npcConfig.forceSave();
        RC.getUserMap().trackUUID(npcUUID, name, false);
    }

    private static void deleteNPC(String name) {
        User user = RC.getUser(name);
        user.reset();
    }
    ...
    public static void setMoney(String name, BigDecimal balance) throws UserDoesNotExistException, NoLoanPermittedException {
        ...
        if (balance.compareTo(RC.getSettings().getMinMoney()) < 0) {
            throw new NoLoanPermittedException();
        }
        if (balance.signum() < 0 && !user.isAuthorized("refinecraft.eco.loan")) {
            throw new NoLoanPermittedException();
        }
        try {
        ...
        Trade.log("API", "Set", "API", name, new Trade(balance, RC), null, null, null, RC);
    }
    ...
    public static void add(String name, BigDecimal amount) throws UserDoesNotExistException, NoLoanPermittedException, ArithmeticException {
        ...
        Trade.log("API", "Add", "API", name, new Trade(amount, RC), null, null, null, RC);
    }
    ...
    public static void substract(String name, BigDecimal amount) throws UserDoesNotExistException, NoLoanPermittedException, ArithmeticException {
        ...
        setMoney(name, result);
        Trade.log("API", "Subtract", "API", name, new Trade(amount, RC), null, null, null, RC);
    }
    ...
    public static void divide(String name, BigDecimal amount) throws UserDoesNotExistException, NoLoanPermittedException, ArithmeticException {
        ...
        setMoney(name, result);
        Trade.log("API", "Divide", "API", name, new Trade(amount, RC), null, null, null, RC);
    }
    ...
    public static void multiply(String name, BigDecimal amount) throws UserDoesNotExistException, NoLoanPermittedException, ArithmeticException {
        ...
        setMoney(name, result);
        Trade.log("API", "Multiply", "API", name, new Trade(amount, RC), null, null, null, RC);
    }
    ...
    public static void resetBalance(String name) throws UserDoesNotExistException, NoLoanPermittedException {
        ...
        setMoney(name, RC.getSettings().getStartingBalance());
        Trade.log("API", "Reset", "API", name, new Trade(BigDecimal.ZERO, RC), null, null, null, RC);
    }
     ...
     */
    @Deprecated
    public static boolean hasLRC(String name, double amount) throws UserDoesNotExistException {
        try {
            return hasLRC(name, BigDecimal.valueOf(amount));
        } catch (ArithmeticException e) {
            logger.log(Level.WARNING, "Failed to compare balance of " + name + " with " + amount + ": " + e.getMessage(), e);
            return false;
        }
    ...
    public static boolean hasLRC(String name, BigDecimal amount) throws UserDoesNotExistException, ArithmeticException {
        return amount.compareTo(getMoneyExact(name)) > 0;
    }
    ...
    /**
     * Formats the amount of money like all other RefineCraft functions. Example: $100000 or $12345.67
     *
    ...
    public static String format(BigDecimal amount) {
        ...
        return NumberUtil.displayCurrency(amount, RC);
    }
