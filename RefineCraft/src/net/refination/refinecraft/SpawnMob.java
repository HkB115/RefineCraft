...
import net.refination.refinecraft.Mob.MobException;
...
public class SpawnMob {
    public static String mobList(final User user) {
        ...
        for (String mob : mobList) {
            if (user.isAuthorized("refinecraft.spawnmob." + mob.toLowerCase(Locale.ENGLISH))) {
    ...
    public static void spawnmob(final InterfaceRefineCraft RC, final Server server, final User user, final List<String> parts, final List<String> data, int mobCount) throws Exception {
        final Block block = LocationUtil.getTarget(user.getBase()).getBlock();
        if (block == null) {
            throw new Exception(tl("unableToSpawnMob"));
        }
        spawnmob(RC, server, user.getSource(), user, block.getLocation(), parts, data, mobCount);
    }

    // This method spawns a mob at target, owned by target
    public static void spawnmob(final InterfaceRefineCraft RC, final Server server, final CommandSource sender, final User target, final List<String> parts, final List<String> data, int mobCount) throws Exception {
        spawnmob(RC, server, sender, target, target.getLocation(), parts, data, mobCount);
    }

    // This method spawns a mob at loc, owned by target
    public static void spawnmob(final InterfaceRefineCraft RC, final Server server, final CommandSource sender, final User target, final Location loc, final List<String> parts, final List<String> data, int mobCount) throws Exception {
        final Location sloc = LocationUtil.getSafeDestination(loc);

        for (int i = 0; i < parts.size(); i++) {
            Mob mob = Mob.fromName(parts.get(i));
            checkSpawnable(RC, sender, mob);
        }
        ...
        try {
            for (int i = 0; i < mobCount; i++) {
                spawnMob(RC, server, sender, target, sloc, parts, data);
            }
    ...
    private static void spawnMob(final InterfaceRefineCraft RC, final Server server, final CommandSource sender, final User target, final Location sloc, List<String> parts, List<String> data) throws Exception {
        Mob mob;
    ...
    private static void checkSpawnable(InterfaceRefineCraft RC, CommandSource sender, Mob mob) throws Exception {
        ...
        if (sender.isPlayer() && !RC.getUser(sender.getPlayer()).isAuthorized("refinecraft.spawnmob." + mob.name.toLowerCase(Locale.ENGLISH))) {
