...
import net.refination.refinecraft.signs.Signs;
...
public class Settings implements net.refination.api.InterfaceSettings {
    ...
    public Settings(InterfaceRefineCraft RC) {
        this.RC = RC;
        config = new RefineCraftConf(new File(RC.getDataFolder(), "config.yml"));
    ...
    public int getHomeLimit(final User user) {
        int limit = 1;
        if (user.isAuthorized("refinecraft.sethome.multiple")) {
        ...
        if (homeList != null) {
            for (String set : homeList) {
                if (user.isAuthorized("refinecraft.sethome.multiple." + set) && (limit < getHomeLimit(set))) {
    ...
    @Override
    public boolean isCommandDisabled(final InterfaceRefineCraftCommand cmd) {
    ...
    @Override
    public BigDecimal getCommandCost(InterfaceRefineCraftCommand cmd) {
    ...
    private ConfigurationSection _getCommandCosts() {
        if (config.isConfigurationSection("command-costs")) {
            ...
            for (String command : section.getKeys(false)) {
                if (command.charAt(0) == '/') {
                    RC.getLogger().warning("Invalid command cost. '" + command + "' should not start with '/'.");
                }
                if (section.isDouble(command)) {
                    ...
                    try {
                        ...
                        RC.getLogger().warning("Invalid command cost for: " + command + " (" + costString + ")");
                    }

                } else {
                    RC.getLogger().warning("Invalid command cost for: " + command);
                }
            }
            return newSection;
        }
    ...
    public BigDecimal getCommandCost(String name) {
        ...
        if (commandCosts != null) {
            return RefineCraftConf.toBigDecimal(commandCosts.getString(name), BigDecimal.ZERO);
        }
    ...
    public boolean showNonEssCommandsInHelp() {
        return config.getBoolean("non-RC-in-help", true);
    }
    ...
    @Override
    public InterfaceText getAnnounceNewPlayerFormat() {
    ...
    private List<Integer> _getItemSpawnBlacklist() {
        ...
        for (String itemName : config.getString("item-spawn-blacklist", "").split(",")) {
            ...
            try {
                final ItemStack iStack = RC.getItemDb().get(itemName);
    ...
    private List<RefineCraftSign> enabledSigns = new ArrayList<RefineCraftSign>();
    private boolean signsEnabled = false;

    @Override
    public List<RefineCraftSign> enabledSigns() {
        return enabledSigns;
    }

    private List<RefineCraftSign> _getEnabledSigns() {
        this.signsEnabled = false; // Ensure boolean resets on reload.

        List<RefineCraftSign> newSigns = new ArrayList<RefineCraftSign>();
    ...
    public List<Integer> getProtectList(final String configName) {
        ...
        for (String itemName : config.getString(configName, "").split(",")) {
            ...
            ItemStack itemStack;
            try {
                itemStack = RC.getItemDb().get(itemName);
    ...
    private boolean refinecraftChatActive = false;
    ...
    @Override
    public void setRCchatActive(boolean refinecraftChatActive) {
        this.refinecraftChatActive = refinecraftChatActive;
    }

    @Override
    public boolean addPrefixSuffix() {
        return prefixsuffixconfigured ? addprefixsuffix : refinecraftChatActive;
    }
