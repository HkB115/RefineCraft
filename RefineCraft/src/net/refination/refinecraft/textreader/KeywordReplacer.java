...
import net.refination.refinecraft.ExecuteTimer;
import net.refination.refinecraft.PlayerList;
import net.refination.refinecraft.User;
import net.refination.refinecraft.utils.DateUtil;
import net.refination.refinecraft.utils.DescParseTickFormat;
...
public class KeywordReplacer implements InterfaceText {
    private static final Pattern KEYWORD = Pattern.compile("\\{([^\\{\\}]+)\\}");
    private static final Pattern KEYWORDSPLIT = Pattern.compile("\\:");
    private final transient InterfaceText input;
    ...
    public KeywordReplacer(final InterfaceText input, final CommandSource sender, final InterfaceRefineCraft RC) {
        this.input = input;
    ...
    public KeywordReplacer(final InterfaceText input, final CommandSource sender, final InterfaceRefineCraft RC, final boolean showPrivate) {
        this.input = input;
        this.replaced = new ArrayList<String>(this.input.getLines().size());
        this.RC = RC;
    ...
    private void replaceKeywords(final CommandSource sender) {
        ...
        if (RC.getSettings().isDebug()) {
            RC.getLogger().log(Level.INFO, "Keyword Replacer " + timeroutput);
        }
    }
        ...
        try {
            ...
            if (replacer == null) {
                ...
                switch (validKeyword) {
                    ...
                    case BALANCE:
                        if (user != null) {
                            replacer = NumberUtil.displayCurrency(user.getMoney(), RC);
                        }
                        break;
                    case MAILS:
                        if (user != null) {
                    ...
                    case ONLINE:
                        ...
                        replacer = Integer.toString(RC.getOnlinePlayers().size() - playerHidden);
                        break;
                    case UNIQUE:
                        replacer = Integer.toString(RC.getUserMap().getUniqueUsers());
                        break;
                    case WORLDS:
                    ...
                    case PLAYERLIST:
                        ...
                        if (keywordCache.containsKey(validKeyword)) {
                            ...
                            for (String groupName : playerList.keySet()) {
                                ...
                                if (groupUsers != null && !groupUsers.isEmpty()) {
                                    outputList.put(groupName, PlayerList.listUsers(RC, groupUsers, " "));
                                }
                            }
                        ...
                        break;
                    case TIME:
                        replacer = DateFormat.getTimeInstance(DateFormat.MEDIUM, RC.getI18n().getCurrentLocale()).format(new Date());
                        break;
                    case DATE:
                        replacer = DateFormat.getDateInstance(DateFormat.MEDIUM, RC.getI18n().getCurrentLocale()).format(new Date());
                        break;
                    case WORLDTIME12:
                        if (user != null) {
                    ...
                    case WORLDDATE:
                        if (user != null) {
                            replacer = DateFormat.getDateInstance(DateFormat.MEDIUM, RC.getI18n().getCurrentLocale()).format(DescParseTickFormat.ticksToDate(user.getWorld() == null ? 0 : user.getWorld().getFullTime()));
                        }
                        break;
                    case COORDS:
                        if (user != null) {
                        ...
                        break;
                    case TPS:
                        replacer = NumberUtil.formatDouble(RC.getTimer().getAverageTPS());
                        break;
                    case UPTIME:
                    ...
                    case PLUGINS:
                        ...
                        break;
                    case VERSION:
                        replacer = RC.getServer().getVersion();
                        break;
                    default:
                        replacer = "N/A";
                        break;
