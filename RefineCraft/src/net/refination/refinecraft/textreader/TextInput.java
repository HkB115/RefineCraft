...
public class TextInput implements InterfaceText {
    ...
    public TextInput(final CommandSource sender, final String filename, final boolean createFile, final InterfaceRefineCraft RC) throws IOException {

        File file = null;
        if (sender.isPlayer()) {
            final User user = RC.getUser(sender.getPlayer());
            file = new File(RC.getDataFolder(), filename + "_" + StringUtil.sanitizeFileName(user.getName()) + ".txt");
            if (!file.exists()) {
                file = new File(RC.getDataFolder(), filename + "_" + StringUtil.sanitizeFileName(user.getGroup()) + ".txt");
            }
        }
        if (file == null || !file.exists()) {
            file = new File(RC.getDataFolder(), filename + ".txt");
        }
        if (file.exists()) {
                    ...
                    while (bufferedReader.ready()) {
                        ...
                        if (line.length() > 1 && line.charAt(0) == '#') {
                            ...
                            for (String title : titles) {
                                bookmarks.put(FormatUtil.stripRefineCraftFormat(title.toLowerCase(Locale.ENGLISH)), lineNumber);
                            }
                        }
            ...
            if (createFile) {
                final InputStream input = RC.getResource(filename + ".txt");
