...
public class BookInput implements InterfaceText {
    ...
    public BookInput(final String filename, final boolean createFile, final InterfaceRefineCraft RC) throws IOException {
        ...
        if (!file.exists()) {
            if (createFile) {
                ...
                RC.getLogger().info("File " + filename + ".txt does not exist. Creating one for you.");
            }
        }
        if (!file.exists()) {
            lastChange = 0;
