...
import net.refination.refinecraft.RefineCraft;
import net.refination.refinecraft.perm.impl.*;
...
public class PermissionsHandler implements InterfacePermissionsHandler {
    private transient InterfacePermissionsHandler handler = null;
    private transient String defaultGroup = "default";
    private final transient RefineCraft RC;
    private transient boolean useSuperperms = false;

    private Class<?> lastHandler = null;

    public PermissionsHandler(final RefineCraft plugin, final boolean useSuperperms) {
        this.RC = plugin;
    ...
    public void checkPermissions() {
        ...
        for (Class<? extends InterfacePermissionsHandler> providerClass : providerClazz) {
            try {
                InterfacePermissionsHandler provider = providerClass.newInstance();
        ...
        if (handler == null) {
            if (useSuperperms) {
                ...
                handler = new ConfigPermissionsHandler(RC);
            }
        }
        ...
        if (handler instanceof GenericVaultHandler) {
            ...
            RC.getLogger().info("Using Vault based permissions (" + enabledPermsPlugin + ")");
        } else if (handler.getClass() == SuperpermsHandler.class) {
            if (handler.tryProvider()) {
                RC.getLogger().warning("Detected supported permissions plugin " +
                        ((SuperpermsHandler) handler).getEnabledPermsPlugin() + " without Vault installed.");
                RC.getLogger().warning("Features such as chat prefixes/suffixes and group-related functionality will not " +
                        "work until you install Vault.");
            }
            RC.getLogger().info("Using superperms-based permissions.");
        } else if (handler.getClass() == ConfigPermissionsHandler.class) {
            RC.getLogger().info("Using config file enhanced permissions.");
            RC.getLogger().info("Permissions listed in as player-commands will be given to all users.");
        }
    }
    ...
    private void checkPermLag(long start, String summary) {
        ...
        if (elapsed > RC.getSettings().getPermissionsLagWarning()) {
            RC.getLogger().log(Level.WARNING, String.format("Permissions lag notice with (%s). Response took %fms. Summary: %s", getName(), elapsed / 1000000.0, summary));
        }
    }
