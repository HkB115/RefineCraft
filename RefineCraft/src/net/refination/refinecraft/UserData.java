...
public abstract class UserData extends PlayerExtension implements InterfaceConf {
    protected final transient InterfaceRefineCraft RC;
    private final RefineCraftUserConf config;
    private final File folder;

    protected UserData(Player base, InterfaceRefineCraft RC) {
        super(base);
        this.RC = RC;
        folder = new File(RC.getDataFolder(), "userdata");
        if (!folder.exists()) {
            folder.mkdirs();
        }
        ...
        try {
            ...
            RC.getLogger().warning("Falling back to old username system for " + base.getName());
            filename = base.getName();
        }

        config = new RefineCraftUserConf(base.getName(), base.getUniqueId(), new File(folder, filename + ".yml"));
        reloadConfig();
    }
    ...
    public final void reset() {
        ...
        if (config.username != null) {
            RC.getUserMap().removeUser(config.username);
        }
    }
    ...
    private BigDecimal _getMoney() {
        BigDecimal result = RC.getSettings().getStartingBalance();
        BigDecimal maxMoney = RC.getSettings().getMaxMoney();
        BigDecimal minMoney = RC.getSettings().getMinMoney();
    ...
    public boolean isIgnoredPlayer(final String userName) {
        final InterfaceUser user = RC.getUser(userName);
    ...
    public boolean isIgnoredPlayer(InterfaceUser user) {
        return (ignoredPlayers.contains(user.getName().toLowerCase(Locale.ENGLISH)) && !user.isIgnoreExempt());
    }

    public void setIgnoredPlayer(InterfaceUser user, boolean set) {
        if (set) {
    ...
    public void setLastAccountName(String lastAccountName) {
        ...
        config.save();
        RC.getUserMap().trackUUID(getConfigUUID(), lastAccountName, true);
    }
